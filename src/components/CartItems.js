...
const CartItems = () => {
  ...
  for (const sku in cartDetails) {
    ...
    for (
      let quantity = 1;
      quantity <= 20 || quantity <= entry.quantity;
      ++quantity
    )
      options.push(
        <option
          selected={quantity === entry.quantity ? "selected" : ""}
          key={quantity}
          value={quantity}
        >
          {quantity}
      ...
      <div className="columns is-variable is-3 is-mobile">
        ...
        <div className="column">
          ...
          <p className="subtitle is-6 mb-3">
            {entry.quantity} × 500 g • {entry.formattedValue}
          </p>
      ...
      <div className="columns is-variable is-2 is-vcentered is-mobile">
        <div className="column is-narrow">
          <figure className="image">
            ...
            >
              <path
                d="M16 43C24.2843 43 31 36.2843 31 28C31 19.7157 24.2843 13 16 13C7.71573 13 1 19.7157 1 28C1 36.2843 7.71573 43 16 43Z"
                stroke="#8B384B"
                strokeWidth="2"
                strokeMiterlimit="10"
              ...
              />
              <path
                d="M22 28C23.1046 28 24 27.1046 24 26C24 24.8954 23.1046 24 22 24C20.8954 24 20 24.8954 20 26C20 27.1046 20.8954 28 22 28Z"
                fill="#8B384B"
              />
            </svg>
          </figure>
        </div>
        <div className="column">Der Warenkorb ist leer.</div>
      </div>
    </div>
  );
};
